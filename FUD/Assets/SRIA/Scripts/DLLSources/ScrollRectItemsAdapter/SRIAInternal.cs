...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		void OnScrollViewValueChanged (Vector2 _Calue)
		{
		...
		void ComputeVisibility (double abstractDelta)
		{
			...
			if (Math.Abs (abstractDelta) > 10d// huge jumps need optimization
			...
			if (double.IsInfinity (currentItemVrtInset_negStart_posEnd) && currentLVItemIndexInView != endItemIndexInView) { // this check is the same as in the loop below. there won't be no "next" item if the current LV is the last in view (first if positive scroll) 
            ...
            {
                int last = _ItemsDesc.GetItemRealIndexFromViewIndex(_ItemsDesc.realIndexOfFirstItemInView);
                if (last < 0)
                {
                    return;
                }
