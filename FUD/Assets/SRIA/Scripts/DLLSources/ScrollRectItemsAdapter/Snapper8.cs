...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	public class Snapper8 : MonoBehaviour
	{
		...
		bool _SnapNeeded;
		// a new snap will only start if after the las snap the scrollrect's scroll position has changed
		...
		void OnScrolled (float _)
		{
			if (!SnappingInProgress)
				_SnapNeeded = true;
		}
		// from adapter
		void OnScrollbarValueChanged (float _)
		{
			if (IsPointerDraggingOnScrollbar)
				CancelSnappingIfInProgress ();
		}
		// from scrollbar
