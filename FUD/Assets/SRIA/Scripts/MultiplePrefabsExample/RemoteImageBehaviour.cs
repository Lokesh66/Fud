...
namespace frame8.ScrollRectItemsAdapter.MultiplePrefabsExample
{
	...
	public class RemoteImageBehaviour : MonoBehaviour
	{
		...
		void Awake ()
		{
			...
			string directoryPath = Application.temporaryCachePath + "/TBImages";
		...
		{
			LoadSprite (filePath, System.IO.Path.GetFileName (imageURL), (byte[] obj) => {
		...
		public void Load (string imageURL, bool loadCachedIfAvailable = true, Action<bool, bool> onCompleted = null, Action onCanceled = null)
		{
			...
			string filePath = Application.temporaryCachePath + "/TBImages/" + System.IO.Path.GetFileName (imageURL);
			if (File.Exists (filePath)) {
				Destroy (_RawImage.material.mainTexture);

				_RawImage.texture = _LoadingTexture;

				string filesPath = Application.temporaryCachePath + "/TBImages/" + System.IO.Path.GetFileName (imageURL);
				...
				var request = new SimpleImageDownloader.Request () {
					...
					onDone = result => {
						if (!_DestroyPending && imageURL == _CurrentRequestedURL) { // this will be false if a new request was done during downloading, case in which the result will be ignored
							string path = Application.temporaryCachePath + "/TBImages/" + System.IO.Path.GetFileName (_CurrentRequestedURL);
