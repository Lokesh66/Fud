...
namespace frame8.ScrollRectItemsAdapter.Util
{
	...
	public class LazyList<T>
	{
		...
		{
			_BackingList = new List<T> (Array.CreateInstance (typeof(T), newCount) as T[]);
		}

		public void Add (int count)
		{
			_BackingList.AddRange (Array.CreateInstance (typeof(T), count) as T[]);
		}

		public void InsertAtStart (int count)
		{
			_BackingList.InsertRange (0, Array.CreateInstance (typeof(T), count) as T[]);
		}

		public void Insert (int index, int count)
		{
			_BackingList.InsertRange (index, Array.CreateInstance (typeof(T), count) as T[]);
		}
		...
		public class EnumerableLazyList : IEnumerable<T>
		{
			...
			internal EnumerableLazyList (LazyList<T> lazyList)
			{
				_LazyList = lazyList;
			}
			...
			{
				return _LazyList.GetEnumeratorForExistingItems ();
			}
