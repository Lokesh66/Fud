...
namespace frame8.ScrollRectItemsAdapter.GridExample
{
	...
	/// </summary>
	public class StoriesHistoryTableView : GridAdapter<GridParams, StoryHistoryCellHolder>, ILazyListSimpleDataManager<StoryHistoryModel>
	{

		public UnityEngine.Events.UnityEvent OnItemsUpdated;

		private LazyList<StoryHistoryModel> _Data;

		public LazyList<StoryHistoryModel> Data { get { return _Data; } private set { _Data = value; } }

		public StoriesHistoryView adataObject;
		...
		{
			Data = new LazyList<StoryHistoryModel> (CreateNewModel, adataObject.historyModels.Count);
		}
		...
		StoryHistoryModel CreateNewModel (int index)
		{
			return adataObject.historyModels [index];
		}
		...
		protected override void UpdateCellViewsHolder (StoryHistoryCellHolder viewsHolder)
		{
			var model = Data [viewsHolder.ItemIndex];

			viewsHolder.views.gameObject.transform.parent.GetComponent<StoryHistoryCell> ().Load (model);

			//var imageURLAtRequest = model.fromUser.profile_image;
		...
		bool IsRequestStillValid(int itemIndex, int itemIdexAtRequest, string imageURLAtRequest)
		{
			return
				...
				&& imageURLAtRequest == Data[itemIndex].fromUser.profile_image; // be sure the model at that index is the same (could have changed if ChangeItemCountTo would've been called meanwhile)
		}
	...
	public class StoryHistoryCellHolder : CellViewsHolder
	{
