...
namespace frame8.ScrollRectItemsAdapter.GridExample
{
	...
	/// </summary>
	public class StoryShareTableView : GridAdapter<GridParams, ShareProfileCellHolder>, ILazyListSimpleDataManager<UserSearchModel>
	{

		public UnityEngine.Events.UnityEvent OnItemsUpdated;

		private LazyList<UserSearchModel> _Data;

		public LazyList<UserSearchModel> Data { get { return _Data; } private set { _Data = value; } }

		public StoryShareView adataObject;
		...
		{
			Data = new LazyList<UserSearchModel> (CreateNewModel, adataObject.dataList.Count);
		}
		...
		UserSearchModel CreateNewModel (int index)
		{
		...
		protected override void UpdateCellViewsHolder (ShareProfileCellHolder viewsHolder)
		{
			var model = Data [viewsHolder.ItemIndex];

			viewsHolder.views.gameObject.transform.parent.GetComponent<ShareSearchCell> ().SetView (model, adataObject.OnSelectMember, adataObject.OnCellButtonAction);
		...
		bool IsRequestStillValid(int itemIndex, int itemIdexAtRequest, string imageURLAtRequest)
		{
			return
				...
				&& imageURLAtRequest == Data[itemIndex].profile_image; // be sure the model at that index is the same (could have changed if ChangeItemCountTo would've been called meanwhile)
		}
	...
	public class ShareProfileCellHolder : CellViewsHolder
	{
