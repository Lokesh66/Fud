...
namespace frame8.ScrollRectItemsAdapter.GridExample
{
	...
	/// </summary>
	public class AuditionsAlteredTableView : GridAdapter<GridParams, AuditionAlteredCellHolder>, ILazyListSimpleDataManager<JoinedAudition>
	{

		public UnityEngine.Events.UnityEvent OnItemsUpdated;

		private LazyList<JoinedAudition> _Data;

		public LazyList<JoinedAudition> Data { get { return _Data; } private set { _Data = value; } }

		public AuditionAlteredView adataObject;
		...
		{
			Data = new LazyList<JoinedAudition> (CreateNewModel, adataObject.joinedAuditions.Count);
		}
		...
		JoinedAudition CreateNewModel (int index)
		{
			return adataObject.joinedAuditions [index];
		}
		...
		protected override void UpdateCellViewsHolder (AuditionAlteredCellHolder viewsHolder)
		{
			var model = Data [viewsHolder.ItemIndex];

			viewsHolder.views.gameObject.transform.parent.GetComponent<JoinedAuditionCell> ().SetView (adataObject, model);

			var imageURLAtRequest = model.Audition.image_url;

			var itemIndexAtRequest = viewsHolder.ItemIndex;
		...
		bool IsRequestStillValid(int itemIndex, int itemIdexAtRequest, string imageURLAtRequest)
		{
			return
				...
				&& imageURLAtRequest == Data[itemIndex].Audition.image_url; // be sure the model at that index is the same (could have changed if ChangeItemCountTo would've been called meanwhile)
		}
	...
	public class AuditionAlteredCellHolder : CellViewsHolder
	{
		...
		public override void CollectViews ()
		{
			...
			views.GetComponentAtPath("IconImage", out remoteImageBehaviour);
		}
