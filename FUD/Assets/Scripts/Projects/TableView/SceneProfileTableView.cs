...
namespace frame8.ScrollRectItemsAdapter.GridExample
{
	...
	/// </summary>
	public class SceneProfileTableView : GridAdapter<GridParams, SceneProfileCellHolder>, ILazyListSimpleDataManager<SceneCharacter>
	{
		...
		public SceneAlbumView adataObject;
		...
		{
			Data = new LazyList<SceneCharacter> (CreateNewModel, adataObject.sceneCharacters.Count);
		}
		...
		{
			return adataObject.sceneCharacters [index];
		}
		...
		protected override void UpdateCellViewsHolder (SceneProfileCellHolder viewsHolder)
		{
			var model = Data [viewsHolder.ItemIndex];

			viewsHolder.views.gameObject.transform.parent.GetComponent<SceneAlbumCell> ().SetView (model, adataObject.OnCellButtonAction);
	...
	public class SceneProfileCellHolder : CellViewsHolder
	{
